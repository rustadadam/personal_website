					...
					<div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
						{contactMethods.map((method, idx) => (
							<motion.div
								...
								whileHover={{ scale: 1.025, boxShadow: '0 8px 32px 0 rgba(0,128,128,0.10)' }}
								className="flex items-center gap-4 bg-gradient-to-br from-white via-teal-50 to-teal-100 dark:from-gray-800 dark:via-teal-900/30 dark:to-teal-900 rounded-2xl p-5 shadow-md hover:shadow-xl transition-all duration-200"
							>
								{method.href ? (
									<a
										href={method.href}
								...
								<div>
									{method.href ? (
										<a
											...
											target={method.href.startsWith('http') ? '_blank' : undefined}
											rel={method.href.startsWith('http') ? 'noopener noreferrer' : undefined}
											className="text-base font-semibold text-gray-900 dark:text-white mb-1 font-[Poppins,sans-serif] hover:text-coral-500 dark:hover:text-coral-400 transition-colors"
										>
											{method.label}
										</a>
									) : (
										<span className="text-base font-semibold text-gray-900 dark:text-white mb-1 font-[Poppins,sans-serif]">{method.label}</span>
									)}
									{method.href ? (
										<a
											href={method.href}
